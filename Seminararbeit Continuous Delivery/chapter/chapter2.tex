...
\begin{list}{label}{spacing}
	...
	\item[\textbf{N7}] Inhalt des Dokumentes hat nichts mit Continuous Delivery oder den im Kapitel \nameref{subsec:picoc} genannten Begrifflichkeiten zu tun.
...
\begin{description}
	\item[\suchstring{Komplex} =] \suchstringPrefix \\AND\\ ("'Jenkins"' OR "'Deployment"' OR "'Pipeline"' OR "'DevOps"') \\AND\\ ("'Kosten"' OR "'costs"' OR "'Tests"') \\AND\\ ("'Technik"' OR "'techniques"' OR "'Praxis"' OR "'practice"' OR "'Anwendung"' OR "'usage"')
...
\end{tabular}\noindent
\\\\
Der Begriff \textit{Continuous Delivery} ist stark mit dem Bereich \underline{Biologie} verbunden. Daher wurde die Suche mit dem Suchstring \suchstring{einfach} nicht auf Google Scholar angewendet. Gibt man diesen Begriff in Google Scholar ein, so sind die meisten Einträge aus der Biologie.
